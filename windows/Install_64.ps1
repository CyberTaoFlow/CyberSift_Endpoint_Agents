...
if ($sysmon_config -ne ""){
  ...
  Add-Content C:\CyberSift\winlogbeat\winlogbeat-5.2.2-windows-x86_64\winlogbeat.yml "`noutput.elasticsearch:`n  hosts: [`"http://$server`:80/cybersift_elasticsearch/`"]"
...
if ($packetbeat_config -ne ""){
  ...
  Add-Content C:\CyberSift\packetbeat\packetbeat-5.2.2-windows-x86\packetbeat.yml "`noutput.elasticsearch:`n  hosts: [`"http://$server`:80/cybersift_elasticsearch/`"]`n`npacketbeat.interfaces.device: $Interface`n"
